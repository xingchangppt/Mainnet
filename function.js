...
function markscore0(vd,d,t,s,l,ac){
	var alt=['不喜欢！','凑合吧！','还不错！','值得看！','超级棒！'],url=ac=='news' ? ["/"+sitePath+"inc/ajax.asp?id="+vd+"&action=newsscore","/"+sitePath+"inc/ajax.asp?id="+vd+"&action=scorenews&score="] : ["/"+sitePath+"inc/ajax.asp?id="+vd+"&action=videoscore","/"+sitePath+"inc/ajax.asp?id="+vd+"&action=score&score="],
...
function markscore1(vd,d,t,s,l,ac){
	var alt=['评分结果:不好看！','评分结果:凑合吧！','评分结果:挺好的,值得看!','评分结果:极力推荐！','评分结果:百年难遇好片！'],src=['/'+sitePath+'pic/star0.gif','/'+sitePath+'pic/star1.gif'],url=ac=='news' ? ["/"+sitePath+"inc/ajax.asp?id="+vd+"&action=newsscore","/"+sitePath+"inc/ajax.asp?id="+vd+"&action=scorenews&score="] : ["/"+sitePath+"inc/ajax.asp?id="+vd+"&action=videoscore","/"+sitePath+"inc/ajax.asp?id="+vd+"&action=score&score="],
	...
	for(var i=0;i<im.length;i++){
		im[i].onclick=function (){
			...
			ajax.get(url[1]+x,function (obj){
				if((''+obj.responseText).indexOf("havescore")!=-1){
					alert('你已经评过分啦,一天只能评分一次哦！');
				}else{
					alert('感谢你给了这部影片新的生命力！');
					y=x;dc.onmouseout();
				}
			});
		}
